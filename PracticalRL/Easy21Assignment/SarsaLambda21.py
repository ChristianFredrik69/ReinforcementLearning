...
class SarsaLambdaControl:
        ...
        while True:
            ...
            if done:
                ...
                for sa in E:
                    ...
                    self.Q[state][action] += (1/self.N[state][action]) *  (reward - self.Q[state][action]) * E[sa]
            ...
            for sa in E:
                    state, action = sa[0], sa[1]
                    self.Q[state][action] += (1/self.N[state][action]) *  (reward + self.gamma * self.Q[next_state][next_action] - self.Q[state][action]) * E[sa]
    ...
    # This one is not used for Sarsa-lambda, but I'll let it be for now.
